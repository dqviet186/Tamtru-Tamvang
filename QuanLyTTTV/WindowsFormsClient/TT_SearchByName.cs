...
{
    public partial class TT_SearchByName : Form
    {
        public TT_SearchByName()
        {
            InitializeComponent();
        }
        ...
        public DataSet dataTTTV(string Name, string type)
        {
            DataSet ds = new DataSet();

            ServiceReference1.pisClient proxy = new ServiceReference1.pisClient();
            TTTVService.TranferRecord[] result = proxy.FindInfoByName(Name, type);
            ds = CreateData();
            DataRow dr;
        ...
        private void TT_SearchByName_Load(object sender, EventArgs e)
        {
            textBox1.Focus();
        }
    }
